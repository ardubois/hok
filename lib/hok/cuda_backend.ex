defmodule Hok.CudaBackend do
  ...
  ####################### OLD Compiling lambdas
  def gen_lambda_ref(module, list_lambdas) do
    name_code = Enum.map(list_lambdas,fn {:anon, function} -> name = gen_lambda_name(); code = compile_lambda(function,[], name); {"anonymous_#{name}",code} end)
